...
description: Summarize the current coding-agent session, inspect and stage relevant git changes, check and update README.md if it is stale, create an appropriate git commit, then either ask what remote action to take or finish with git push or pull request creation when the user includes the keyword "push" or "pr". Use at the end of a work session when the user wants completed work packaged into version control.
---
...
2. Check README accuracy:
   - Read `README.md` when it exists.
   - Compare it against the current project, code, commands, file layout, and user-facing behavior changed during the session.
   - Update `README.md` before staging if it is stale or omits important new project status.
   - Do not rewrite README content that is still accurate, and do not invent unsupported setup, test, or usage commands.
   - If the repository intentionally has no README, mention that in the final response and continue.
3. Summarize the session before committing:
...
4. Stage only the changes that belong to the completed session:
...
6. Finish according to the invocation mode:
...
- whether `README.md` was checked and updated,
